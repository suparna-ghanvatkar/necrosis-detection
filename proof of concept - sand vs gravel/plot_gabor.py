...
image_names = ('sand_pebble')
#images = (brick, grass, wall)
image = cv2.imread('C:\Users\NISHANT\Desktop\Sem 3\RE\Sand vs Pebble\Sand_new_2.jpg',cv2.IMREAD_GRAYSCALE)

# prepare reference features
ref_feats = np.zeros((1, len(kernels), 2), dtype=np.double)
ref_feats[0, :, :] = compute_feats(image, kernels)
...
feats = compute_feats(ndi.rotate(image, angle=190, reshape=False), kernels)
...
'''
for label, img, ax in zip(image_names, image, axes[0][1:]):
    ax.imshow(img)
...
for i in range(len(results)):
    cv2.imshow('resp'+str(i),results[i][1])
    cv2.waitKey(0)
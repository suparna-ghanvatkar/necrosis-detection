...
'''
pathNecrosis = os.path.join(os.path.dirname(__file__),'dataset/necrosis')
pathNonNecrosis = os.path.join(os.path.dirname(__file__),'dataset/non-necrosis')
...
classifiers = [svm.SVC(kernel = 'rbf'), KNeighborsClassifier(n_neighbors=11), KNeighborsClassifier(n_neighbors=10), tree.DecisionTreeClassifier(), RandomForestClassifier(), GradientBoostingClassifier() ]
names = ["svm", "knn11", "knn10", "dtree", "randforest", "gboost"]
knn_clf = range(3,30)
#clf = svm.SVC(kernel = 'rbf')
for i,n in enumerate(knn_clf):
	clf = KNeighborsClassifier(n_neighbors=n)
	for score in scores:
		s = cross_val_score(clf, X_feat[1:], np.array(Y), scoring = score, cv=10)
		print str(n), score, s.mean()
